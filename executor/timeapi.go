...
func (e *Executor) post(status interface{}) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		// Token rejected by server — clear it so the next attempt triggers a fresh login
		if err == nil && (resp.StatusCode == 401 || resp.StatusCode == 403) {
			log.Printf("[POST] Token rejected (HTTP %d), clearing for re-login...", resp.StatusCode)
			e.token = ""
		}

		log.Printf("[POST] Attempt %d/%d failed: %v", attempt, maxRetries, err)
		time.Sleep(1 * time.Second)
	}
